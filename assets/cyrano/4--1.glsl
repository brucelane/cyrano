// https://www.shadertoy.com/view/4d2XD1
//Procedural Cubemaps by nimitz (twitter: @stormoid)

/*
	Follow up on my "sphere mappings" shader. Using said mapping to draw a procedural
	4-way symmetrical texture.
...
{    
	...
	col = col*col*.5;
    col *= sin(length(p)*9.+time*4.)*0.35+0.65;
	return col;
}
...
{	
    ...
    mat2 mx = mm2(sTime*0.25+um.x*5.);
    mat2 my = mm2(sTime*0.27+um.y*5.);
    ro.xz *= mx;rd.xz *= mx;
    ro.xy *= my;rd.xy *= my;

    float sel = mod(floor(sTime*0.3),4.);
    ...
    col *= sin(bp.y*450.*iResolution.y+sTime*0.1)*0.02+1.;
    col *= (texture2D(iChannel0,p*0.003*iResolution.x+fract(sTime)*vec2(51,100)).r-0.5)*.1+1.;
    #endif
