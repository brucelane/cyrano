...
float triangle(vec2 p, float size) {
    vec2 q = abs(p);
    return max(q.x * RATIO + p.y * 0.5, -p.y * 0.5) - size * 0.5;
}
...
{   //vec2 uv = inData.v_texcoord;
    ...
    float tint = triangle(uv - vec2(0.5, TY - 0.02), TLNG - 0.05 ) ;
    color = blanc * ( 1 - smoothstep( 0, text, tint ) ) ;

    // Le trait blanc
    float xg = 2 * x ;
    // On fait varier RBB en fonction du temps :
    float RBB = IRBB + sin( itime ) * 0.5 ;
    float RBH = IRBH + cos( itime / 2 ) * 0.01 ;
